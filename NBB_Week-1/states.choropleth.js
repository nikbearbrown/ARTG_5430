...
const files = ['data/maps/us-states.geo.json', 'data/mortality_cardiovascular_diseases_states.json'];
const promises = [];
Promise.all([
    d3.json(files[0]),
    d3.json(files[1])
...
function choroplethiseMap (pathSelection, data) {
    ...
    pathSelection.style('fill', function(d) {
        ...
        console. log(data)
        let region = data.filter(e => e.FIPS === d.properties.STATE);
